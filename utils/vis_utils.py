...
def open3dVisualize(mList, colorList):
    ...
    # # added code below
    # for m in o3dMeshList:
    #     raw_input()
    #     print(np.asarray(m.vertices)[np.asarray(m.triangles)].shape)

    
    raw_input("Enter for hand")
    h_m = o3dMeshList[0]
    o_m = o3dMeshList[1]
    open3d.visualization.draw_geometries([h_m])
    print(h_m.vertices)
    raw_input("Enter for obj")
    open3d.visualization.draw_geometries([o_m])
